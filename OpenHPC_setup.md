...
```bash
sudo hostnamectl set-hostname nodename.local
...
8. Update the slurm configurations for the PackwoodGPU computer using ``sudo vim /etc/slurm/slurm.conf``.
    ...
    * **IMPORTANT**: Make sure you change all instances of ``nodename`` to the hostname of the PackwoodGPU Computer. You can get this by entering in ``hostname`` into the terminal, which will give you ``nodename.local``. 
...
#
# Nodes
NodeName=nodename NodeAddr=nodename NodeHostName=nodename Gres=gpu:2 CPUs=6 Sockets=1 CoresPerSocket=6 ThreadsPerCore=1 RealMemory=120000  State=UNKNOWN
#
# Partitions
PartitionName=gpu Nodes=nodename State=UP MaxTime=INFINITE
PartitionName=cpu Nodes=nodename State=UP MaxTime=INFINITE

```

9. Add the ``gres.conf`` file for GPU allocation using ``sudo vim /etc/slurm/gres.conf``.

    * Please check number of GPU, e.g. nvidia[0-1] is 2 GPUs.
    * **IMPORTANT**: Make sure you change all instances of ``nodename`` to the hostname of the PackwoodGPU Computer. You can get this by entering in ``hostname`` into the terminal, which will give you ``nodename.local``. 

```bash title="sudo vim /etc/slurm/gres.conf"
NodeName=packwoodgpu Name=gpu File=/dev/nvidia[0-1]
```
